// z3p3.cpp : Defines the entry point for the console application.
//
#include "stdafx.h"
...
void GenerateTextures()
{
	...
	GLboolean error = load_targa("uranus.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku uranus.tga");
		exit(0);
	}
	...
	error = load_targa("uranus.tga", width, height, format, type, pixels);
...
int main(int argc, char *argv[])
{
	...
	glutAddMenuEntry("Uran", VENUS_TEX);
	glutAddMenuEntry("Uran", EARTH_TEX);
	glutAddMenuEntry("Uran", MARS_TEX);
