// z4p1.cpp : Defines the entry point for the console application.
//
...
void GenerateTextures()
{
	...
	GLboolean error = load_targa("whiteskimman.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku whiteskimman.tga");
		exit(0);
	}
	...
	error = load_targa("whiteskimman.tga", width, height, format, type, pixels);
...
int main(int argc, char *argv[])
{
	...
	glutAddMenuEntry("whiteskimman.tga (kompresja)", TEXTURE_LENA);
	glutAddMenuEntry("whiteskimman.tga (bez kompresji)", TEXTURE_LENA_UNC);
	glutAddMenuEntry("whiteskimman_gray.tga (kompresja)", TEXTURE_LENA_GRAY);
	glutAddMenuEntry("whiteskimman_gray.tga (bez kompresji)", TEXTURE_LENA_GRAY_UNC);
